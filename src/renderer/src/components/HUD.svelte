<script lang="ts">
  ...
  // Initialize with the prop to prevent a pulse on first load
  ...
  onMount(() => {
    // If we are the chosen manager, claim the flag (for internal logic if needed later)
  ...
  // --- VISUALS ONLY (Pulse) ---
  $effect(() => {
    // We removed the 'new Notification()' code here to prevent duplicates.
    // Now we ONLY check for the visual pulse.
    if (ready && isOverlay) {
