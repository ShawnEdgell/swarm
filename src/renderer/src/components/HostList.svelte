...
<div class="flex flex-col gap-3 w-full pb-10">
  ...
  {:else}
    {#each onlineUsers as user (user.id)}
      ...
      <div
        class="border-2 rounded-2xl p-5 transition-all group relative overflow-hidden {flair.cardClass} shadow-xl hover:border-white/20"
      >
        <div class="flex items-center justify-between gap-6">
          ...
          <div class="flex items-center gap-3 shrink-0 py-1">
            <div class="text-right flex flex-col items-end">
              {#if flair.label}
                <span
                  class="text-[8px] font-black text-primary uppercase tracking-widest leading-none mb-1"
                >
                  {flair.label}
                </span>
              {/if}
            ...
            <img
              ...
              alt=""
              class="w-11 h-11 rounded-xl bg-base-300 border-2 {flair.ringClass} shadow-lg"
            />
          </div>
        </div>
      </div>
